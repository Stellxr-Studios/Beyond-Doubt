Shader "Custom/DissolveSurfaceShader" 
{
	...
	SubShader 
	{
		...
		//static float dAdjust = dMag; //( _DissolveBand * clamp(_GlowScale, 1.0, 100.0) - _DissolveBand) / dMag;

		//Convert dissolve progression to new space accounting for band size and glow prediction.
		static half dConverted = _DissolveScale * dMag;//_DissolveScale * (1.0f + dAdjust) - dAdjust;

		//Precompute gradient start position.
		static float3 dissolveStartConverted = _DissolveStart; //_DissolveStart - _DissolveBand * dDir;
		...
		void vert (inout appdata_full v, out Input o) 
		{
			...
			float3 dPoint = lerp(_DissolveStart, _DissolveEnd, dConverted);//lerp(dissolveStartConverted, _DissolveEnd, dConverted);

			//Project vector between current vertex and top of gradient onto dissolve direction.
			//Scale coefficient by band (gradient) size.
			o.dGeometry = dot(v.vertex - dPoint, dDir); //dot(v.vertex - dPoint, dDir) * dBandFactor;		
		}
